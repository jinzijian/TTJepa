...
mean ± sample standard deviation。LeWM 是原始 non-recurrent
baseline；Fixed $K=1,2,3,4$ 和 dynamic $K$ 来自同一组 RefineJEPA
checkpoints。

例如 Reacher dynamic $K$ 的三个 seed 分别是 84%、90%、82%，因此写成
**85.3±4.2%**。这里的 `±` 表示 seed 之间的 sample standard deviation，
不是 95% confidence interval，也不是 episode-level error bar。
